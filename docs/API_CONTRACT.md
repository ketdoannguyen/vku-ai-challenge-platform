...
### 5.7 Kiểm tra notebook bằng AI (ADR-036, ADR-037, ADR-040, ADR-043, ADR-045)
...
| GET | `/api/admin/submissions/{submission_id}/ai-review` | implemented (ADR-036) | Chi tiết một bài: `{submission:{id,submission_no,status,created_at,account,competition}, ai_review, content_snapshot, history}`. `history` là các lượt mới nhất trước, mỗi lượt có verdict/model_verdict/summary/`participant_summary`/findings/bằng chứng/host/model/versions/cache/duration/`downgrade_codes`/`error` đã che. Bằng chứng là đoạn notebook **server tự trích lại**, không phải output model. Không có raw prompt, raw response, `object_key` hay API key. `versions` gồm **sáu** khoá `{prompt, normalization, context_policy, canonicalization, rule_ref, verifier}`; ba khoá cuối (ADR-045) trả `null` cho audit row ghi trước Hybrid B+D, và client phải đọc thiếu-field là "không biết", **không** phải "đã xác minh". Mỗi finding mang thêm các field hậu kiểm của ADR-045: `rule_ref` (ref đã resolve, `null` khi không resolve được), `model_rule_ref` (ref model gửi, giữ nguyên để đối chiếu), `rule_resolution` (`REFERENCE`/`CANONICAL_QUOTE`/`UNRESOLVED`), `rule_verified`, `evidence_count`/`valid_evidence_count`, `evidence_verified`, `verified`, `traceable`, `verification_codes`. `source_content_title`/`source_content_slug`/`rule_text` **luôn** do backend điền từ revision; chúng rỗng khi và chỉ khi quy định không resolve được - model không còn đường nào đưa văn xuôi của mình vào ba field đó. `rule_quote` của model **không** xuất hiện trong response vì nó chỉ là khoá tra cứu, không bao giờ được lưu. |
...
- Nhóm AI review (ADR-036) - **chỉ admin thấy**; participant không bao giờ nhận mã kỹ thuật nào dưới đây:
  ...
  - 422/409 điều kiện chạy: `AI_REVIEW_DISABLED`, `AI_REVIEW_NOT_AVAILABLE`, `AI_NOTEBOOK_MISSING`, `AI_CONTENT_SNAPSHOT_UNAVAILABLE` (cũng dùng khi revision không dựng được chỉ mục quy định - ví dụ trùng `page_slug` - và pipeline dừng **trước** khi gọi provider, ADR-045), `AI_CONTENT_TOO_LARGE` (422); `AI_REVIEW_IN_PROGRESS` (409)

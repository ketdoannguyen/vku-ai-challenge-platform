...
| Method | Path | Status | Mô tả |
|---|---|---|---|
| GET | `/api/admin/accounts` | implemented | Query: `q` (search email/name, email exact match nếu dạng email), `limit` (1-200, default 50), `offset` (≥0). `limit`/`offset` ngoài khoảng → 422 `VALIDATION_ERROR`. Trả `{accounts: [safe fields], total, limit, offset}`. |
...
- `CONFIRM_SLUG_MISMATCH` (422), `COMPETITION_NOT_DELETABLE` (409), `MEMBER_HAS_SUBMISSIONS` (409) — implemented cho luồng xoá giữ lịch sử (ADR-018)

...
| GET | `/api/admin/competitions/{id}/submissions` | implemented (Sprint 06; sort từ ADR-028, metric sort từ ADR-029, `review` từ ADR-035) | Query `q` (tên/email), `status` (`completed|rejected|failed` - **trạng thái chấm điểm**), `review` (`accepted|rejected` - **trạng thái duyệt**, trục riêng), `sort` (`created_at`\|`team`\|`primary_score`\|`f1`\|`precision`\|`recall`, default `created_at`), `order` (`asc`\|`desc`, default `desc`), `limit` (1-200, default 50), `offset`. Trả `{submissions:[{id,competition_id,submission_no,status,metrics,primary_score,created_at,artifacts,error?,account:{id,name,email},review}],total,limit,offset,sort,order}` trong đúng competition. Không trả `object_key`/`file_path`. `sort`/`order`/`review` sai → 422 `VALIDATION_ERROR`. Không nhận `sort=competition` (vô nghĩa khi đã khoá theo một cuộc thi) và **không** trả `stats`. |
